...
const Checkout = () => {
  ...
  useEffect(() => {
    fetch(`https://one-click-shopping-server.vercel.app/product/${id}`)
      .then((res) => res.json())
  ...
  const handlePaymentCheckout = (paymentDetails) => {
    fetch("https://one-click-shopping-server.vercel.app/checkoutOrder", {
      method: "POST",
      body: JSON.stringify({
        ...orderedProduct,
        paymentDetails,
    ...
    <Container className="mt-5">
      ...
      {product.name ? (
        <>
          {!orderedProduct ? (
            <div>
              ...
              <Button
                ...
                className="d-block ml-auto mb-3 mt-4 mr-2"
                variant="success"
              >
                Checkout
              </Button>
          ...
          {isOrdered && (
            <h5 className="mt-3 text-success text-center">
              Ordered Successful
            </h5>
          )}
          {isOrdered && (
