...
const Admin = ({ products, setProducts }) => {
  ...
  const handleEdit = (product, index, setIsEdit) => {
    ...
    if (name && price && weight) {
      fetch(
        `https://one-click-shopping-server.vercel.app/updateProduct/${_id}`,
        {
          method: "PATCH",
        ...
        .then((result) => {
          if (result) {
            ...
            setIsEdit ? setIsEdit(false) : history.push(`${url}/manageProduct`);
          }
        });
    }
  };
          ...
          <Navbar.Collapse className="navbar-section" id="basic-navbar-nav">
            <Nav className="side-navbar flex-column">
              <Link onClick={() => handleActive("manage")} to="/admin">
                <Navbar.Brand className="mb-3 text-white">
                  One Click Shopping
                </Navbar.Brand>
              </Link>
              <Link
                onClick={() => handleActive("manage")}
                className={`p-2 nav-link text-white ${active.manage}`}
                to={`${url}/manageProduct`}
              >
                {" "}
                <FontAwesomeIcon icon={faThLarge} /> Manage Products
              </Link>
              <Link
                onClick={() => handleActive("add")}
                className={`p-2 nav-link text-white ${active.add}`}
                to={`${url}/addProduct`}
              >
                {" "}
              ...
              <Link
                ...
                className={`p-2 nav-link text-white ${active.orders}`}
                to={`${url}/allOrders`}
              >
                {" "}
                <FontAwesomeIcon icon={faSort} /> All Orders
              </Link>
            </Nav>
          </Navbar.Collapse>
        </Navbar>
